...
export function PrescriptionsPanel({ patientId, doctorId, patientDisplayName, patientPhone }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6" aria-labelledby="rx-heading">
      ...
      {/* Mobile cards */}
      <div className="mt-4 space-y-3 pb-2 md:hidden">
        {loading ? (
          ...
          rows.map((r) => (
            <article key={r.id} className="mobile-card">
              ...
              <MobileCardActions className="rx-mobile-actions">
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => void queuePrint(r)}
                  className="bg-teal-50 text-teal-800 disabled:opacity-50"
                >
                  <Printer className="size-5 shrink-0" aria-hidden />
                  <span>{t('print.printShort')}</span>
                </button>
                <button
                  type="button"
                  disabled={busy || editingId !== null}
                  onClick={() => startEdit(r)}
                  className="bg-slate-100 text-slate-700 disabled:opacity-40"
                >
                ...
                <button
                  ...
                  className="bg-rose-50 text-rose-700 disabled:opacity-50"
                >
